...
export default async function WhaleTradesPage({
  ...
  /*
   * صفحة صفقات الحيتان تعتمد الآن على
   * Institutional Flow V2 فقط.
   *
   * شروط الفرصة:
   * - Sweep أو Block
   * - اتجاه التنفيذ BUY
  ...
  const filteredTrades = hydratedTrades.filter((trade) => {
    ...
    const isInstitutionalV2 =
      ...
      estimatedSide === "BUY" &&
      (
        executionLocation === "ASK" ||
        executionLocation === "AT_ASK"
      ) &&
      score >= 75;

    if (!isInstitutionalV2) {
      return false;
    }

    /*
     * فلاتر المستخدم الاختيارية فقط،
     * وليست شروط النظام القديم.
     */
    if (
      symbolFilter &&
      !symbol.includes(symbolFilter)
    ) {
      return false;
    }
